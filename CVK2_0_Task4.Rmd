...
data_Sy <- fromJSON("/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/Task4/Task4_System_content.txt")

# data system reflection (system assessment + reflection task in week 2) 
# task name: Task3_Mirroring
data_M <- fromJSON("/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/Task4/Task4_Mensch_content.txt")


### SEQUENCES ###
# unnest sequences from lists to data.frame
sequences_Sy <- data_Sy$group_sequences %>%
  unnest()
...
sequences <- bind_rows(list(sequences_Sy, sequences_M)) %>% 
  filter(!content %like% "ich die Rückmeldung zum nächsten")


#write.csv2(sequences, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_activities.csv", row.names = F)

### WORK IMBALANCE ###
# extract work imbalances
work_imbalance_Sy <- data_Sy$work_imbalance
work_imbalance_M <- data_M$work_imbalance


# combine all files to a single table
work_imbalances <- bind_rows(list(work_imbalance_Sy, work_imbalance_M))

# adding condition information #
work_imbalances <- addConditionsCVK2(work_imbalances)

#write.csv2(work_imbalances, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_work_imbalances.csv", row.names = F)

### FINAL WIKI weighted wordcounts ###
weighted_wordcount_wiki_Sy <- data_Sy$weighted_wiki_wordcount %>%
  unnest()
weighted_wordcount_wiki_M <- data_M$weighted_wiki_wordcount %>%
  unnest()

# combine all files to a single table
weighted_wiki_wordcounts <- bind_rows(list(weighted_wordcount_wiki_Sy, weighted_wordcount_wiki_M)) %>%
  addConditionsCVK2() 

#write.csv2(weighted_wiki_wordcounts, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_weighted_wiki_wordcounts.csv", row.names = F)

### FINAL Forum WORD COUNTS ###
wordcount_share_forum_Sy <- data_Sy$weighted_forum_wordcount %>%
  unnest()
wordcount_share_forum_M <- data_M$weighted_forum_wordcount %>%
  unnest()

# combine all files to a single table
forum_shares <- bind_rows(list(wordcount_share_forum_Sy, wordcount_share_forum_M)) %>%
  addConditionsCVK2()

#write.csv2(forum_shares, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_forum_shares.csv", row.names = F)

# extract groups and users
users_groups_Sy <- data_Sy$model_metadata$groups %>%
  unnest()
users_groups_M <- data_M$model_metadata$groups %>%
  unnest()

# combine all files to a single table
groups_and_users <- bind_rows(list(users_groups_Sy, users_groups_M)) %>%
...
#write.csv2(groups_and_users, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_groups_and_users.csv", row.names = F)
```
...
#write.csv2(sequences2, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_activities.csv")
...
# Task 4
# task_start:   1575284400    (02.12.19 11:00 AM UTC) (12:00 local)
# task_end:     1576451100    (15.12.19 23:05 PM UTC) (00:05 local)


# start + end timestamps
task_start <- 1575284400
task_end <- 1576451100
...
task_start_activity <- data.frame(as.list(c(group_id=1, content="start", object_id="start", object_name="start", object_type="start", timestamp=task_start, user_id="start", verb_id="start", wordcount=0, condition="test", day="2019-12-02", period=1, type="start", class="start")))
...
addStartElement <- function(df) {
  ...
  if(df[1,]$group_id %in% c(553:565)) {
    sequence[1,]$timestamp <- as.numeric(as.character(task_start_activity$timestamp))
    sequence[1,]$day <- "2019-12-02"
    sequence[1,]$period <- 1
  }
...
#write.csv2(all_sequences_with_start, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/t4_activities_with_start.csv", row.names = F)

all_sequences <- read.csv2("/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/t4_activities_with_start.csv", stringsAsFactors = F) %>% addTaskCVK2()
...
groups_and_users <- read.csv2("/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_groups_and_users.csv", stringsAsFactors = F) %>% 
  mutate(wordcount = 0)
...
#write.csv2(changing_gini, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_changing_gini.csv", row.names = F)
...
#write.csv2(changing_activity_count, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_changing_activity_count.csv", row.names = F)
```
...
#write.csv2(changing_word_count, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_changing_word_count.csv", row.names = F)
```
...
# write.csv2(overall_wordcounts, "/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task4_group_wordcounts.csv", row.names = F)
```
