...
```{r}
gini_data <- read.csv2("/home/doberstein/Dropbox/D/uni/IKARion/CVK 2_0/analysis/task2_work_imbalances.csv", stringsAsFactors = F)

wordcounts_memberships <- full_join(wordcounts_memberships, gini_data)
```
...
```{r}
classified_dataset_fh <- all_sequences %>% 
  filter(period <= 7)
...
all_sequences_fh <- bind_rows(classified_dataset_fh, half_time_activities) %>% 
  group_by(group_id) %>% 
